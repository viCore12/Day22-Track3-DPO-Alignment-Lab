...
DPO_OUT = REPO_ROOT / "adapters" / f"dpo-b{BETA}"
PREF_PATH = REPO_ROOT / "data" / "pref" / "train.parquet"

# ── W&B Bonus (+2 rigor add-on) ─────────────────────────────────────────────
# Initializes W&B run if WANDB_API_KEY is set. Falls back silently if not set.
...
if USE_WANDB:
    try:
        ...
        wandb.init(
            project=os.environ.get("WANDB_PROJECT", "lab22-dpo"),
            name=f"dpo-beta{BETA}-lr{LR}-{COMPUTE_TIER}",
            config={
                ...
                "epochs": EPOCHS,
                "max_length": MAX_LEN,
                "pref_slice": 2000 if COMPUTE_TIER == "T4" else 5000,
            },
        )
...
print(f"W&B logging: {'ENABLED → ' + str(WANDB_RUN_URL) if USE_WANDB else 'DISABLED'}")
...
if USE_WANDB:
    try:
        ...
        print(f"W&B finish error (non-fatal): {e}")
